#!/usr/bin/env bash
# Serve one GiGPO v3i checkpoint via peft_chat_server on a dedicated GPU/port,
# run the AppWorld dev eval, then stop the server. Clone of eval_binary_ckpt.sh
# (vanilla) pointed at the gigpo checkpoint, so GiGPO-vs-vanilla numbers come
# from an identical eval path.
# Usage: eval_gigpo_ckpt.sh <STEP|final> <GPU> <PORT>
set -euo pipefail
STEP="$1"; GPU="$2"; PORT="$3"
...
if [ "$STEP" = "final" ]; then
  LORA="grpo/ckpts/gigpo_9b_full_binary_v3i/round1"
  NAME="gigpo_9b_v3i_step76"; CFG="gigpo_9b_v3i_step_76"
else
  LORA="grpo/ckpts/gigpo_9b_full_binary_v3i/round1/step_${STEP}"
  NAME="gigpo_9b_v3i_step${STEP}"; CFG="gigpo_9b_v3i_step_${STEP}"
fi
