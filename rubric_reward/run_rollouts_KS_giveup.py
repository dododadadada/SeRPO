"""
run_rollouts_KS_giveup.py — Joint segmentation + per-segment reward scoring
with abandoned-subgoal TRUNCATION applied as a post-processor.

Variant of run_rollouts_KS_baseline.py. Differences:
- Prompt is identical to baseline.
- After scoring, walks the segments and truncates everything AFTER the
  first failed segment (contribution <= 2) whose successor has a
  different subgoal AND a different type (i.e., the agent abandoned the
  subgoal and moved on without retrying).
  The failed segment itself is KEPT — it's the negative training signal
  the policy needs to learn from. Only the segments strictly after it
  are dropped. trajectory_text, num_steps, and contribution stats are
  recomputed on the kept prefix.
- Adds two audit fields to each saved record:
    truncated_at_step          (int or null; end_step of the failed segment)
    truncated_segments_dropped (int, 0 if untruncated)
- Writes to results/rollout/round0_9b/KS_giveup/seed_<N>.jsonl and
  round0_9b/KS_giveup/pretty/ so it doesn't collide with the baseline's
...
def truncate_at_abandoned_failure(
    segments: list[dict],
) -> tuple[list[dict], Optional[int]]:
    """Truncate everything AFTER the first failed-and-abandoned segment.

    A segment is "failed" if its `contribution` <= 2. It is "abandoned" if the
    next segment has a different subgoal AND a different type — i.e., the agent
    moved on to unrelated work rather than retrying.

    The failed segment itself is KEPT (it's the negative signal the policy
    must learn from). Everything strictly AFTER it is dropped.

    Returns (kept_segments, cut_at_step_or_None) where cut_at_step is the
    end_step of the last KEPT segment (= the failed segment's end_step in
    the truncated case). The caller uses this to re-derive trajectory_text
    and num_steps consistently. If no truncation applies, returns
    (segments, None).
    """
    for i, seg in enumerate(segments):
        if seg.get("contribution", 5) > 2:
            continue
        if i + 1 >= len(segments):
            continue  # failed segment is already last; nothing to truncate
        nxt = segments[i + 1]
        same_subgoal = (
            str(seg.get("subgoal", "")).strip().lower()
            == str(nxt.get("subgoal", "")).strip().lower()
        )
...
def process_rollout(
    ...
    merged_segments = merge_consecutive_same_score(valid_segs)
    num_segments_pre_truncation = len(merged_segments)
    segments, cut_at_step = truncate_at_abandoned_failure(merged_segments)
    truncated_segments_dropped = num_segments_pre_truncation - len(segments)

    if cut_at_step is not None:
        steps_kept = [s for s in steps if s["step"] <= cut_at_step]
        trajectory_text = format_trajectory(steps_kept)
        num_steps = max((s["step"] for s in steps_kept), default=0)
    else:
    ...
    return {
        ...
        "truncated_at_step": cut_at_step,
        "truncated_segments_dropped": truncated_segments_dropped,
