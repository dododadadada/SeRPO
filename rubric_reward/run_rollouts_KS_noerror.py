"""
run_rollouts_KS_noerror.py — KS variant with `error_recovery` removed as a phase type.

Derived from run_rollouts_KS_baseline.py. The only prompt change vs baseline is that
recoveries are tagged by their underlying activity (`api_exploration`, `data_fetch`, ...)
rather than as a separate `error_recovery` type. The recovery context lives in the
`rationale`, not the `type`.
...
For each rollout, calls GPT-5-mini (OpenAI direct) with the joint prompt to produce
segments with per-segment contribution scores (1-5). Writes results to
rubric_reward/results/rollout/round0_9b/KS_noerror/seed_<N>.jsonl (one file per
seed), with pretty per-record samples in round0_9b/KS_noerror/pretty/ when
--limit is set.
...
A "phase" is one coherent activity. There are six phase types:
